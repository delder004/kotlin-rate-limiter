...
class EdgeCaseTest {
    ...
    @Test
    fun `bursty - high-rate limiter handles rapid acquires`() = runTest {
    ...
    @Test
    fun `smooth - high-rate limiter handles rapid acquires`() = runTest {
    ...
    @Test
    fun `bursty - large acquire request delays proportionally`() = runTest {
    ...
    @Test
    fun `smooth - large acquire request delays proportionally`() = runTest {
    ...
    @Test
    fun `tryAcquire after partial refill returns correct retryAfter`() = runTest {
        val limiter = BurstyRateLimiter(10, 1.seconds, testTimeSource)
        limiter.acquire(10)
    ...
    @Test
    fun `bursty - rapid tryAcquire calls do not consume permits`() = runTest {
    ...
    @Test
    fun `smooth - rapid tryAcquire calls do not consume permits`() = runTest {
    ...
    @Test
    fun `composite - three limiters respects most restrictive`() = runTest {
    ...
    @Test
    fun `composite - mixed bursty and smooth respects smooth spacing`() = runTest {
        val limiter = CompositeRateLimiter(
            BurstyRateLimiter(5, 1.seconds, testTimeSource),
