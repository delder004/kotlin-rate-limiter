...
class CompositeRateLimiterTest : RateLimiterContractTest() {
    ...
    @Test
    fun `acquire succeeds when all limiters have capacity`() = runTest {
    ...
    @Test
    fun `acquire suspends when any limiter is exhausted`() = runTest {
    ...
    @Test
    fun `respects most restrictive limiter`() = runTest {
        val limiter = CompositeRateLimiter(
            BurstyRateLimiter(10, 1.seconds, testTimeSource),
    ...
    @Test
    fun `tryAcquire returns Denied if any limiter denies`() = runTest {
        val fast = BurstyRateLimiter(10, 1.seconds, testTimeSource)
        val slow = BurstyRateLimiter(1, 1.seconds, testTimeSource)
        val limiter = CompositeRateLimiter(fast, slow)
    ...
    @Test
    fun `tryAcquire returns longest retryAfter`() = runTest {
    ...
    @Test
    fun `both limiters consume permits on acquire`() = runTest {
    ...
    @Test
    fun `layered limits work correctly`() = runTest {
