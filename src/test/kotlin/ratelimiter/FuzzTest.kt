...
class FuzzTest {
    ...
    @Test
    fun `bursty - total delay matches permits consumed with random batch sizes`() = runTest {
        val capacity = 100
        val per = 1.seconds
    ...
    @Test
    fun `smooth - total delay matches permits consumed with random batch sizes`() = runTest {
        val capacity = 100
        val per = 1.seconds
        val intervalMs = (per / capacity).inWholeMilliseconds // 10ms
    ...
    @Test
    fun `bursty - no permits lost with concurrent random access`() = runTest {
        val capacity = 50
        val per = 1.seconds
    ...
    @Test
    fun `bursty - tryAcquire denials do not affect acquire timing`() = runTest {
    ...
    @Test
    fun `smooth - tryAcquire denials do not affect acquire timing`() = runTest {
    ...
    @Test
    fun `random interleaving of acquire and tryAcquire is consistent`() = runTest {
        ...
        assertTrue(currentTime >= minExpectedTime,
