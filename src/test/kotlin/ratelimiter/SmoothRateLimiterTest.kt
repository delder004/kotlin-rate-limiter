...
class SmoothRateLimiterTest : RateLimiterContractTest() {
    override fun TestScope.createLimiter(permits: Int, per: Duration): RateLimiter {
        return SmoothRateLimiter(permits, per, Duration.ZERO, testTimeSource)
    }

    // BASIC SMOOTH BEHAVIOR

    @Test
    fun `first permit returned immediately, subsequent permits delayed even when available`() = runTest {
    ...
    @Test
    fun `permits are evenly spaced`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        val delays = recordDelays(limiter, 5)
    ...
    @Test
    fun `spacing is consistent under steady load`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `no burst accumulation during idle`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `permits issued never exceed one window of work`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        advanceTimeBy(5.seconds)
        runCurrent()
    ...
    @Test
    fun `tryAcquire Denied includes correct retryAfter`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        limiter.acquire(5)
    ...
    @Test
    fun `idle accumulation capped at 1`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        advanceTimeBy(5.seconds)
    ...
    @Test
    fun `idle time is not penalized`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        val before = currentTime
        limiter.acquire(1)
    ...
    @Test
    fun `acquire after long idle returns immediately`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `acquire(n) consumes multiple permits`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `acquire(n) charges proportionally for next caller`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `acquire(n) where n more than permits per window`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
