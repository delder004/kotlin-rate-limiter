...
class BurstyRateLimiterTest : RateLimiterContractTest() {
    override fun TestScope.createLimiter(permits: Int, per: Duration): RateLimiter {
        return BurstyRateLimiter(permits, per, testTimeSource)
    }

    // BASIC BURST BEHAVIOR

    @Test
    fun `acquire immediately returns when permits available`() = runTest {
    ...
    @Test
    fun `acquire immediately returns all available permits but delays subsequent calls by refill rate`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        val before = currentTime
    ...
    @Test
    fun `burst up to permit count after idle`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        limiter.acquire(5)
    ...
    @Test
    fun `burst never exceeds one window of work`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
    ...
    @Test
    fun `idle accumulation capped at permit count`() = runTest {
        val limiter = createLimiter(5, 1.seconds)
        advanceTimeBy(5.seconds)
        val before = currentTime
        limiter.acquire(5)
