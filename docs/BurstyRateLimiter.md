...
- **`refund(permits)` is available.** The return type is `RefundableRateLimiter`, which adds a `refund()` method to return permits to the bucket if an operation is abandoned before completion.
