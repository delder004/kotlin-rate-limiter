...
- **`refund(permits)` is available.** The factory returns [RefundableRateLimiter](RefundableRateLimiter.md), which adds `refund()` to return permits to the bucket if an operation is abandoned before completion.
