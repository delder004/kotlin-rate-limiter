...
- **`refund(permits)` is available.** The factory returns [RefundableRateLimiter](RefundableRateLimiter.md), which adds `refund()` so permits can be returned if work is abandoned.
- **Cancellation rewind is exact when uncontended.** With `warmup = Duration.ZERO` the rewind is exact under any contention. With warmup enabled, an `acquire()` that captures a pre-reservation snapshot and is cancelled before any other caller mutates the limiter rewinds exactly. If concurrent callers have moved the limiter's tail in the meantime, the cancellation falls back to a bounded best-effort tail-adjust: schedule drift is at most `permits * (coldInterval - stableInterval)`, and the heat accumulator subtracts only the exact delta this reservation contributed. Drift cools off through normal idle decay.
